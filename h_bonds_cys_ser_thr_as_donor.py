...
def calculator_and_writer(structure, desired_donor_atom, desired_hydrogen_atom, relevant_neighbor_dist_cutoff, pdb_id_molprobity, dict_geom_crit, dict_no_neighbors, dictdata, dict_resolutions, list_disulfides, molprobity_files, dict_donor_and_hydrogen_atoms_ON, dict_donor_and_hydrogen_atoms_S, dict_acceptor_and_antecedent_atoms_ON, 
	...
	if desired_donor_atom.get_name() in dict_donor_and_hydrogen_atoms_ON.keys() and desired_acceptor_atom.get_name() == "SD" and desired_acceptor_atom.get_parent().get_resname() == "MET":
		...
		vector_accep_hydrogen_normalized = vector_accep_hydrogen / np.linalg.norm(vector_accep_hydrogen)                       #no need to normalize "vector_accep_ant1" and "vector_accep_ant2" as they aren't required anymore
		vector_bisector_normalized       = vector_bisector       / np.linalg.norm(vector_bisector)

		dot_product_normalized                     = np.dot(vector_accep_hydrogen_normalized, vector_bisector_normalized)      #dot product of vector SD-H and bisector vector (required to calculate the angle between them)
		...
		angle_rad                                  = np.arccos(dot_product_normalized / (magnitude_vector_accep_hydrogen_normalized * magnitude_vector_bisector_normalized)) #angle between vector SD-H and bisector vector
		angle_deg                                  = np.degrees(angle_rad)
		angle_HAAn                                 = angle_deg

	if segments_relevant_chain != None: # it will be equal to None for globular dataset and hence this step will be skipped
		flag2 = False                   # it will become True only if the acceptor atom lies in one of the HBCIL ranges of the current chain
		for segment_range in segments_relevant_chain["HBCIL"]:
			...
			if desired_acceptor_atom.get_parent().get_full_id()[3][1] in range(lower_limit_of_segment_range, upper_limit_of_segment_range + 1):
				flag2 = True
	...
	if distance_DA <= distance_DA_geom and distance_HA <= distance_HA_geom and angle_DHA >= angle_DHA_geom and angle_HAAn >= angle_HAAn_geom:
		...
		dictdata[desired_donor_atom.get_parent().get_resname()].setdefault("DHA angle (degrees)",                   []).append(round(angle_DHA, 2)) 
		dictdata[desired_donor_atom.get_parent().get_resname()].setdefault("HAAn angle/HABisector angle (degrees)", []).append(round(angle_HAAn, 2))
		...
		if segments_relevant_chain != None: # it will be equal to None for globular dataset and hence this step will be skipped
			dictdata[desired_donor_atom.get_parent().get_resname()].setdefault("Acceptor residue belongs to TM segment of the same chain", []).append(flag2)
		...
		if stride_return_value == False: # this happens when the stride file corresponding to the PDB structure isn't found
			dictdata[desired_donor_atom.get_parent().get_resname()].setdefault("Donor sec. str.",    []).append("Stride file not found") # I have used "Stride file not found" and not "NA" for a reason
			dictdata[desired_donor_atom.get_parent().get_resname()].setdefault("Acceptor sec. str.", []).append("Stride file not found")
		if stride_return_value != False:
			dictdata[desired_donor_atom.get_parent().get_resname()].setdefault("Donor sec. str.",    []).append(stride_return_value["Donor ssec"])
			dictdata[desired_donor_atom.get_parent().get_resname()].setdefault("Acceptor sec. str.", []).append(stride_return_value["Acceptor ssec"])
		...
		for key, value in phi_psi_chi1_return_value.items(): # it will look like: {"phi" : {"donor" : "None", "acceptor" : "None"}, {"psi" : {"donor" : "None", "acceptor" : "None"}, {"chi1" : {"donor" : "None", "acceptor" : "None"}}
			for acc_don, anglez in value.items():
				dictdata[desired_donor_atom.get_parent().get_resname()].setdefault(key + " angle " + acc_don + " (degrees)",    []).append(anglez)
		...
		for key, value in sasa_return_value.items():
			dictdata[desired_donor_atom.get_parent().get_resname()].setdefault(key, []).append(value)
	...
	if len(neighbors) != 0:
		for neighbor in neighbors:
			if (neighbor.get_parent().get_full_id()[3][0] == " " and # if neighbor atom's parent's residue is a non-heteroatom entity 
				...
				if neighbor.get_name() in dict_acceptor_and_antecedent_atoms_ON.keys():
...
#^########################################################################################################################################################################################################################################################################
def min_dist_checker(dictionary, desired_donor_atom, desired_acceptor_atom): # this function ensures that in case the acceptor atom is also bonded to a H-atom, the DA distance is still the smallest (compared to the distances between D and any H-atom bonded to the acceptor) 
